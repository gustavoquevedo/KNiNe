...
//import org.apache.spark.rdd.Dataset
...
case class Hash(values: Array[Int]) {
...
trait Hasher extends Serializable {
  ...
  def hashData(data: Dataset[(Long, LabeledPoint)], radius: Double): Dataset[(Hash, Long)]
}
...
trait AutotunedHasher extends Hasher {
  ...
  def getHasherForDataset(data: Dataset[(Long, LabeledPoint)], dimension: Int, desiredComparisons: Int): (EuclideanLSHasher, Int, Double)

  def getHasherForDataset(data: Dataset[(Long, LabeledPoint)], desiredComparisons: Int): (EuclideanLSHasher, Int, Double) =
    getHasherForDataset(data, data.rdd.map({ case (index, point) => point.features.size }).max(), desiredComparisons)
}
...
object EuclideanLSHasher extends AutotunedHasher //with Logging
{
  ...
  private def computeBestKeyLength(data: Dataset[(Long, LabeledPoint)], dimension: Int, desiredComparisons: Int): (EuclideanLSHasher, Double) = {
  ...
  def getSuitableRadius(data: Dataset[(Long, LabeledPoint)], hasher: EuclideanLSHasher, minValue: Double, maxValue: Option[Double], desiredCount: Int): Double = {
    ...
    while (true) {
      ...
      if ((numBuckets == 0) || (largestBucketSize < MIN_TOLERANCE * desiredCount))
        leftLimit = radius
      else if (largestBucketSize > MIN_TOLERANCE * desiredCount) {
        rightLimit = radius
        /*
          //DEBUG
  ...
  override def getHasherForDataset(data: Dataset[(Long, LabeledPoint)], dimension: Int, desiredComparisons: Int): (EuclideanLSHasher, Int, Double) = {
  ...
  override def hashData(data: Dataset[(Long, LabeledPoint)], radius: Double): Dataset[(Hash, Long)] = {
    import data.sparkSession.implicits._
    val bt = data.sparkSession.sparkContext.broadcast(this)
...
class EuclideanLSHasher(dimension: Int, kLength: Int, nTables: Int, splitW: Double = 4.0) extends Hasher {
  ...
  final def hashData(data: Dataset[(Long, LabeledPoint)], radius: Double): Dataset[(Hash, Long)] = {
    import data.sparkSession.implicits._
  ...
  def getBucketCount(data: Dataset[(Long, LabeledPoint)], radius: Double): (Long, Int) = {
    import data.sparkSession.implicits._

    //println("DEBUG: Pre hashData call")
    val currentHashes: Dataset[(Hash, Long)] = this.hashData(data, radius)

    val bucketCountBySize = currentHashes
      .groupByKey(_._1).mapValues(_ => 1).reduceGroups(_ + _)
      .groupByKey(_._2).mapValues(_ => 1).reduceGroups(_ + _)
      .filter(_._1 != 1)
    ...
    */
    ...
    val numBuckets = if (bucketCountBySize.isEmpty) 0 else bucketCountBySize.map(_._2).reduce(_ + _).toLong
    val largestBucketSize = if (bucketCountBySize.isEmpty) 0 else bucketCountBySize.map(_._1).reduce(Math.max(_, _))
...
class EuclideanProjectedLSHasher(dimension: Int, kLength: Int, nTables: Int, blockSz: Int) extends Hasher {
  ...
  final override def hashData(data: Dataset[(Long, LabeledPoint)], radius: Double): Dataset[(Hash, Long)] = {
    import data.sparkSession.implicits._
...
class PrecomputedProjectedLSHasher(kLength: Int, blockSz: Int) extends Hasher {
  ...
  final override def hashData(data: Dataset[(Long, LabeledPoint)], radius: Double): Dataset[(Hash, Long)] = {
    ...
    val t = this
    val bt = data.sparkSession.sparkContext.broadcast(t)
    //println("DEBUG: Pre hashData flatMap call")
